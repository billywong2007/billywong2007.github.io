...
jQuery(document).ready(function ($) {
   ...
   */
  var is_fixed_header =
    $(".site-header.sticky-header").length > 0 ? true : false;
  ...
  if (is_fixed_header) {
    $(".site-header.sticky-header")
      .eq(0)
      .wrap('<div class="site-header-wrapper">');
    ...
    $(window).scroll(function () {
      ...
      if (is_fixed_header && is_transparent) {
        if ($(".swiper-slider").length) {
          start =
            $(".swiper-slider").eq(0).offset().top +
            $(".swiper-slider").outerHeight();
          start = start - header_h - topbar;
        } else if ($(".page-header-cover").length) {
          start =
            $(".page-header-cover").eq(0).offset().top +
            $(".page-header-cover").outerHeight();
          start = start - header_h - topbar;
        }
      }
  ...
  function get_header_top_height() {
    var hh =
      ...
      (jQuery(".site-header.sticky-header").height() || 0);
  ...
  if (string_to_bool(Screenr.is_home_front_page)) {
    ...
    jQuery(window).scroll(function () {
      ...
      jQuery(".site-main section").each(function () {
        ...
        if (
          ...
          s.offset().top - header_top_height - 10
        ) {
    ...
    jQuery("#site-navigation li.menu-item-type-custom").removeClass(
      "current-menu-item"
    );
  }
  ...
  function set_swiper_full_screen_height() {
    ...
    if (
      ...
      !$(".site-header").hasClass("transparent")
    ) {
  ...
  if (
    ...
    var slider_overlay_opacity =
      $(".swiper-slider.fixed .swiper-container .overlay")
        .eq(0)
        .css("opacity") || 0.35;
    $(window).scroll(function () {
      var scrolled = $(window).scrollTop();
  ...
  $(window).resize(function () {
    if (swiper && swiper.length) {
      swiper.container
        .find(".swiper-slide-intro, .btn-next-section")
        .removeAttr("data-top")
        .removeAttr("style");
